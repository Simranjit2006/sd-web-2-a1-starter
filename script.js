...
function renderCharacterList(array, targetId) {
  const list = document.getElementById(targetId);
 
  array.forEach(character => {
    if (!character.name) {
      console.error("Missing name property:", character);
      displayError("error-messages", "Character is missing a name property.");
      return;
    }
    const li = document.createElement("li");
    li.textContent = character.name;
...
renderCharacterList(brokenCharacters, "broken-array-list");
 
brokenCharacters.forEach(character => {
  if (!character.name) {
    displayError(
      "broken-array-errors",
      `Error: Character with ID ${character.id} is missing a name property.`
    );
  }
});
